...
function StopNPCJob(cancel)
    ...
    if cancel then
	  ESX.ShowNotification(_U('cancel_mission'))
		...
		TriggerServerEvent('esx_poolcleanersecondary:GiveItem')
		StartNPCJob()
		Done = true
	end
end
...
AddEventHandler('esx_poolcleanersecondary:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_poolcleanersecondary:hasExitedMarker', function(zone)

	if zone == 'Vente' then
		TriggerServerEvent('esx_poolcleanersecondary:stopVente')
	end
    CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if PlayerData.job2 ~= nil then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_poolcleanersecondary:hasEnteredMarker', currentZone)
			end
			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_poolcleanersecondary:hasExitedMarker', LastZone)
			end
		end
	end
end)
            ...
            if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and PlayerData.job2 ~= nil then
				...
				if PlayerData.job2.name == Config.nameJob2 then
					...
					if CurrentAction == 'vente' then
						TriggerServerEvent('esx_poolcleanersecondary:startVente')
					end

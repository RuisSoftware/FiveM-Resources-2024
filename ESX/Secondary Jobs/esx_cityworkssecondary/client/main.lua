...
AddEventHandler('esx_cityworkssecondary:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_cityworkssecondary:hasExitedMarker', function(zone)

	if zone == 'Vente' then
		TriggerServerEvent('esx_cityworkssecondary:stopVente')
	end
    CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
...
function CreateBlip()
    if PlayerData.job2 ~= nil and PlayerData.job2.name == Config.nameJob then
	...
	if PlayerData.job2 ~= nil and PlayerData.job2.name == Config.nameJob and onDuty then
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job2 ~= nil then
			...
			if PlayerData.job2.name == Config.nameJob then
				if onDuty then
	...
	while true do
		...
		if PlayerData.job2 ~= nil then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_cityworkssecondary:hasEnteredMarker', currentZone)
			end
			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_cityworkssecondary:hasExitedMarker', LastZone)
			end
		end
	end
end)
            ...
            if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and PlayerData.job2 ~= nil then
				...
				if PlayerData.job2.name == Config.nameJob then
					...
					if CurrentAction == 'vente' then
						TriggerServerEvent('esx_cityworkssecondary:startVente')

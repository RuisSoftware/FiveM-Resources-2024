...
function IsJobTrucker()
	if PlayerData ~= nil then
		...
		if PlayerData.job2.name ~= nil and PlayerData.job2.name == 'trucker' then
			isJobTrucker = true
		end
		return isJobTrucker
	end
...
AddEventHandler('esx_truckerjobsecondary:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_truckerjobsecondary:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()    
    CurrentAction = nil
    CurrentActionMsg = ''
end)
...
function donnerlapaye()
	...
	if vievehicule >= 1 then
		if livraisonTotalPaye == 0 then
			...
			TriggerServerEvent("esx_truckerjobsecondary:pay", amount)
	...
	while true do
		...
		if IsJobTrucker() then
			...
			if isInMarker and not hasAlreadyEnteredMarker then
				...
				TriggerEvent('esx_truckerjobsecondary:hasEnteredMarker', currentZone)
			end

			if not isInMarker and hasAlreadyEnteredMarker then
				hasAlreadyEnteredMarker = false
				TriggerEvent('esx_truckerjobsecondary:hasExitedMarker', lastZone)
			end

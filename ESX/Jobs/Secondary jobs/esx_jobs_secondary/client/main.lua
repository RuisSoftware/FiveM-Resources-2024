...
AddEventHandler('esx_secondaryjobs:action', function(job, zone, zoneIndex)
	menuIsShowed = true
	...
	elseif zone.Type == 'work' then
		...
		if IsPedOnFoot(playerPed) then
			TriggerServerEvent('esx_secondaryjobs:startWork', zoneIndex)
		else
			ESX.ShowNotification(_U('foot_work'))
		end
	elseif zone.Type == 'vehspawner' then
		local jobObject, spawnPoint, vehicle = Config.Jobs2[PlayerData.job2.name]
	...
	elseif zone.Type == 'vehdelete' then
		local jobObject = Config.Jobs2[PlayerData.job2.name]
					...
					if IsPedInAnyVehicle(playerPed, false) then
						...
						if playerPed == driverPed then
							if myPlate[plate] then
								...
								TriggerServerEvent('esx_secondaryjobs:caution', 'give_back', giveBack, 0, 0)
...
AddEventHandler('esx_secondaryjobs:hasExitedMarker', function(zone)
	TriggerServerEvent('esx_secondaryjobs:stopWork')
	hintToDisplay = nil
	menuIsShowed = false
	hintIsShowed = false
	isInMarker = false
		...
		if jobObject then
			for k,v in pairs(jobObject.Zones) do
				if v.Blip then
					...
					AddTextComponentSubstringPlayerName(PlayerData.job2.label)
					exports['DP_Blip_Info']:SetBlipInfoTitle(blip, "Bijbaan", rockstarVerified)
					exports['DP_Blip_Info']:AddBlipInfoName(blip, v.Name, "")
					exports['DP_Blip_Info']:AddBlipInfoHeader(blip, "Gradatie:", PlayerData.job2.grade_label)
					exports['DP_Blip_Info']:SetBlipInfoImage(blip, "world_blips", "supermarkt")
					exports['DP_Blip_Info']:AddBlipInfoText(blip, "Dit is je bijbaan", "")
					exports['DP_Blip_Info']:AddBlipInfoText(blip, "Type", "Werk")
					exports['DP_Blip_Info']:AddBlipInfoIcon(blip, "Opdracht", "", "", 9, false)
...
function spawnVehicle(spawnPoint, vehicle, vehicleCaution)
	hintToDisplay = nil
	hintIsShowed = false
	TriggerServerEvent('esx_secondaryjobs:caution', 'take', vehicleCaution, spawnPoint, vehicle)
end

RegisterNetEvent('esx_secondaryjobs:spawnJobVehicle')
AddEventHandler('esx_secondaryjobs:spawnJobVehicle', function(spawnPoint, vehicle)
	local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job2 then
			if Config.Jobs2[PlayerData.job2.name] then
				zones = Config.Jobs2[PlayerData.job2.name].Zones
			end

			for k,v in pairs(zones) do
				local distance = GetDistanceBetweenCoords(playerCoords, v.Pos.x, v.Pos.y, v.Pos.z, true)

				if onDuty or v.Type == 'cloakroom' or PlayerData.job2.name == 'reporter' then
			...
			if IsControlJustReleased(0, 38) and not menuIsShowed and isInMarker then
				if onDuty or currentZone.Type == 'cloakroom' or PlayerData.job2.name == 'reporter' then
					TriggerEvent('esx_secondaryjobs:action', PlayerData.job2.name, currentZone, currentZoneIndex)
				end
			end
			...
			if isInMarker and not menuIsShowed then
				hintIsShowed = true
				if (onDuty or currentZone.Type == 'cloakroom' or PlayerData.job2.name == 'reporter') and currentZone.Type ~= 'vehdelete' then
					hintToDisplay = currentZone.Hint
					hintIsShowed = true
				elseif currentZone.Type == 'vehdelete' and (onDuty or PlayerData.job2.name == 'reporter') then
					local playerPed = PlayerPedId()
			...
			if not isInMarker and hasAlreadyEnteredMarker then
				...
				TriggerEvent('esx_secondaryjobs:hasExitedMarker', currentZone)
			end
		end

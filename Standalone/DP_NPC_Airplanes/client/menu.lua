if Config.ESXMenu then
	Config.Airports = {
		{label = Config.Locale.LSA, type = 'lsa'},
		{label = Config.Locale.SSA, type = 'ssa'},
		{label = Config.Locale.CPA, type = 'cpa'}
	}
	function OpenMenu()
		local elements = {}
		for i=1, #Config.Airports, 1 do
			table.insert(elements, {
				label = Config.Airports[i].label,
				type  = Config.Airports[i].type
			})
		end

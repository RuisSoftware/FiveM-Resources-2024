...
for (let i = 0, length = GetNumResourceMetadata("DP_Radio", "supersede_radio"); i < length; i++) {
    const radio = GetResourceMetadata("DP_Radio", "supersede_radio", i);
    ...
    try {
        const data = JSON.parse(GetResourceMetadata("DP_Radio", "supersede_radio_extra", i));
        if (data !== null) {
            customRadios.push({
                "isPlaying": false,
                "name": radio,
